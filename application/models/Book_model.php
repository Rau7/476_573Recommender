<?php 
    class Book_model extends CI_Model {
        ...
        function get_book($book_id){
            return $this->db->query("SELECT * FROM book WHERE book_id='".(int)$book_id."' LIMIT 1")->row(0,"array");
        }
        ...
        return $this->db->query("INSERT INTO books SET book_name=".$this->db->escape($data['name']).",book_author=".$this->db->escape($data['author']).", book_genre=".$this->db->escape($data['genre']).",book_year=".$this->db->escape($data['year'])."");
        }

        function get_book_year($book_id){
            return $this->db->query("SELECT book_year FROM books WHERE book_id='".(int)$book_id."'")->row(0,"array");
        }

        function get_book_name($book_id){
            return $this->db->query("SELECT book_name FROM books WHERE book_id='".(int)$book_id."'")->row(0,"array");
        }
        ...
        function get_book_id_name($book_name){
            return $this->db->query("SELECT book_id FROM books WHERE book_name='".$book_name."'")->row(0,"array");
        }
        ...
        function update_recs($data,$admin_id){
            return $this->db->query("UPDATE admin SET admin_future_id='".$data."' WHERE admin_id='".$admin_id."'");
        }
